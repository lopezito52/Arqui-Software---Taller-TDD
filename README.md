# Taller TDD - Registraduría

Proyecto de ejemplo para aplicar TDD (Test-Driven Development) con Arquitectura Limpia en un sistema de registraduría electoral.

## 🎯 Objetivos del Taller

- Aplicar TDD siguiendo el ciclo Red → Green → Refactor
- Implementar pruebas unitarias con patrón AAA (Arrange-Act-Assert)
- Utilizar clases de equivalencia y valores límite para diseño de pruebas
- Alinear pruebas con BDD usando Given-When-Then
- Garantizar calidad del dominio sin acoplamiento a frameworks externos

## 📋 Reglas de Negocio Implementadas

| Regla | Validación | Resultado |
|-------|-----------|-----------|
| Estado vital | Persona debe estar viva | `DEAD` si no está viva |
| Edad mínima | ≥ 18 años | `UNDERAGE` si es menor |
| Edad máxima | ≤ 120 años | `INVALID_AGE` si excede |
| Identificador | ID único y positivo | `DUPLICATED` o `INVALID` |
| Validación defensiva | Persona no nula | `INVALID` si es null |
...
│       │   ├── Person.java          # Entidad principal
│       │   └── RegisterResult.java  # ENUM: resultados de registro
│       └── service/
│           └── Registry.java        # Lógica de negocio
└── test/java/edu/unisabana/tyvs/
    └── domain/service/
        └── RegistryTest.java        # Pruebas unitarias
```
...
# Ejecutar todo el ciclo de vida
mvn clean verify
```

## 📊 Reportes de Cobertura

Después de ejecutar `mvn jacoco:report`, ver el reporte en:
```
target/site/jacoco/index.html
```

**Objetivo de cobertura:** ≥ 80% en clases de dominio

## 🧪 Metodología TDD Aplicada

### 🔴🟢🔵 Ciclo Red-Green-Refactor

| Iteración | Prueba | Implementación | Refactor |
|-----------|--------|----------------|----------|
| 1 | Persona válida | Retorno `VALID` | - |
| 2 | Persona muerta | Validación `isAlive()` | Extraer constantes |
| 3 | Persona nula | Validación defensiva | Mejorar legibilidad |
| 4 | Menor de edad | Validación edad mínima | - |
| 5 | Edad excesiva | Validación edad máxima | - |
| 6 | ID duplicado | Registro de IDs | - |
...
## 📈 Matriz de Pruebas - Clases de Equivalencia

| Caso de Prueba | Entrada | Resultado Esperado | Método de Test |
|----------------|---------|-------------------|----------------|
| Camino feliz | Persona(30, true, id=1) | `VALID` | `shouldRegisterValidPerson` |
| Persona fallecida | Persona(40, false, id=2) | `DEAD` | `shouldRejectDeadPerson` |
| Persona nula | null | `INVALID` | `shouldReturnInvalidWhenPersonIsNull` |
| Menor de edad (17) | Persona(17, true, id=3) | `UNDERAGE` | `shouldRejectUnderageAt17` |
| Edad mínima (18) | Persona(18, true, id=4) | `VALID` | `shouldAcceptAdultAt18` |
| Edad máxima (120) | Persona(120, true, id=5) | `VALID` | `shouldAcceptMaxAge120` |
| Edad excesiva (121) | Persona(121, true, id=6) | `INVALID_AGE` | `shouldRejectInvalidAgeOver120` |
| ID duplicado | Mismo ID dos veces | `DUPLICATED` | `shouldRejectDuplicateId` |
| ID inválido (0) | Persona(25, true, id=0) | `INVALID` | `shouldRejectWhenIdIsZeroOrNegative` |
...
Escenario: Registrar persona válida
  Dado que existe una persona viva de 25 años con ID único
  Cuando intento registrarla en la registraduría
...
Ver archivo `defectos.md` para el seguimiento de issues encontrados durante el desarrollo.
...
- Desarrollo guiado por pruebas desde el inicio
- Ciclo continuo: Red → Green → Refactor
- Código validado constantemente

### ✅ Arquitectura Limpia
- Dominio aislado de frameworks externos
- Dependencias hacia adentro
- Fácil de probar y mantener

### ✅ Clases de Equivalencia
- Partición del dominio de entrada
- Valores representativos por clase
- Cobertura con mínimo número de pruebas

### ✅ Valores Límite
- Pruebas en bordes entre clases (17/18, 120/121)
- Donde suelen ocurrir la mayoría de errores

## 🔍 Criterios de Calidad

- Todas las pruebas usan patrón AAA
- Nomenclatura clara en métodos de test (`should...`)
- Un assert principal por test
- Cobertura ≥ 80%
- Código limpio y sin duplicación
- Constantes extraídas (`MIN_AGE`, `MAX_AGE`)
...
**🎓 Universidad de Sabana**  
**📅 2025**  
*Taller de TDD y Pruebas Unitarias*
